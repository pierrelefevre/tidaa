...
function Friends(props) {
    ...
    var search = (e) => {
        if (e)
            e.preventDefault();
        setLoading(true);
        axios.get(API_URL.user + '/users/search?query=' + searchString, { token: props.user.token })
    ...
    var getFriends = () => {
        ...
        axios.get(API_URL.user + '/users/' + props.user.id + "/friends", { token: props.user.token })
            .then(function (response) {
                setLoading(false);
                if (response.status === 200) {
                    console.log(response.data);
    ...
    var addFriend = (friend) => {
        axios.post(API_URL.user + '/users/' + props.user.id + "/friends/" + friend.id, { token: props.user.token })
            .then(function () {
                getFriends();
                setSearchError("");
            })
    ...
    var removeFriend = (friend) => {
        axios.delete(API_URL.user + '/users/' + props.user.id + "/friends/" + friend.id, { token: props.user.token })
            .then(function () {
                getFriends();
                setFriendError("");
            })
    ...
    var renderProfilePic = (friend) => {

        if (friend.imageUrl != null) {
            return (< Avatar src={"https://metaberse-imageapi.herokuapp.com" + friend.imageUrl} sx={{ bgcolor: red[500] }} aria-label="recipe" />);
        }
        else {
            var cached = userCache.getById(friend.id);
            if (cached != null && cached.imageUrl != null) {
                return (< Avatar src={"https://metaberse-imageapi.herokuapp.com" + cached.imageUrl} sx={{ bgcolor: red[500] }} aria-label="recipe" />);
            }
            return (<Avatar sx={{ bgcolor: red[500] }} aria-label="recipe"> {friend.username.charAt(0)} </Avatar>);
        }
                    ...
                    {result.map((friend) => {
                        ...
                        return (
                            <Card sx={{ textAlign: "left", mb: 2 }} key={friend.id}>
                                <CardHeader
                                    avatar={renderProfilePic(friend)}
